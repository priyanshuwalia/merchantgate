...
import { auditEvents, campaigns as campaignsTable, db } from "@/db";
...
export async function updateCampaign(
  ...
  if (patch.startsAt !== undefined)
    updates.starts_at = new Date(patch.startsAt);
 ...
 * payable). Uses atomic SQL increment to avoid read-modify-write races, batched
 * into a single `UPDATE ... FROM (VALUES ...)` so an n-line quote is one
 * round-trip instead of an n-LOOP of updates.
 */
export async function recordCampaignSpend(
  ...
  const rows = clean.map(
    (s) => sql`(${s.campaignId}::text, ${Number(s.discountMinor)}::bigint)`,
  );

  await db
    .update(campaignsTable)
    .set({
      spent_minor: sql`${campaignsTable.spent_minor} + data.spent`,
      updated_at: new Date(),
    })
    .from(
      sql`(VALUES ${sql.join(rows, sql.raw(", "))}) AS data(id text, spent bigint)`,
    )
    .where(sql`${campaignsTable.id} = data.id`);
}
