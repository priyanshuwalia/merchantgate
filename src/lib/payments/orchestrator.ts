...
export async function preparePayment(params: {
  ...
  /** Fresh-quote path: the caller just wrote cart + decision rows, so skip
   * their redundant re-reads and the existing-intent lookup. Saves 3 DB
   * round-trips on the checkout hot path. */
  knownCart?: typeof cartMandates.$inferSelect;
  knownDecision?: typeof policyDecisions.$inferSelect;
}) {
  const [cart] =
    params.knownCart !== undefined
      ? [params.knownCart]
      : await db
          .select()
          .from(cartMandates)
  ...
  const [decision] =
    params.knownDecision !== undefined
      ? [params.knownDecision]
      : await db
          .select()
